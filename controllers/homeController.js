...
function isAvailability(checkin, checkOut, home){
    ...
    resç.forEach(element => {
        if(dateCheckBetween(checkin, checkOut, element.checkIn) || 
            dateCheckBetween(checkin, checkOut, element.checkOut) || 
            dateCheckBetween(element.checkIn, element.checkOut, checkin)){
            return false;
        }
    });
    return true;
